...
**利用递归检索的思路**是：先将该课程设为不可修完，即visit[cur]=-1。然后由graph依次遍历该课程的先修课，若都能修完，则将该课程设为能修完，
即1，返回**true**；若其中有一个不能修完，则直接返回**false**。  
能修完**最底层的条件**为该课程没有先修课或是前面已经检索过了能修完，即**visit[cur]=1**；  
不能修完**最底层的条件**为课程之间围成环，即**visit[cur]=-1**。
## 代码(DFS)：
``` C
class Solution {
public:
...
```
## 解题思路 2：
本题还可以用BFS的办法解决，用一个有向图graph记录课程，即**先修课指向后修课**，用一个栈pre存储**没有**先修课的课程，
